  ...
  <div class="container-xxl flex-grow-1 container-p-y">
		...
		<div
		  ...
		  <div
			...
			<button id="button" class="w3-button btn-primary" style="width: 140px; border-radius: 5px;  ">Export
			  CSV</button>
			  <a href="<?=base_url('dashboard/admin/add_career')?>"><button id="button" class="w3-button btn-primary" style="width: 140px; border-radius: 5px;  "><i class="fa fa-plus" aria-hidden="true"></i>Add</button></a>
		  </div>
		  </div>
    ...
    $(document).ready(function () {
        ...
        function format(details) {
            ...
            return `
                ...
                <strong>Actions: </strong> 
                    <a href="${baseURL}dashboard/admin/edit_career/${btoa(details.id)}">
        ...
        const table = $('#Example').DataTable({
            ...
            scrollCollapse: true,
            ajax: {
                url: '<?= base_url("dashboard/admin/get_careers") ?>',
                type: 'POST',
                dataSrc: 'data'
            },
            columns: [
        ...
        $('#Example').on('change', '.toggle-status', function () {
            ...
            $.ajax({
                url: '<?=base_url('dashboard/admin/change_career_status')?>',
                type: 'POST',
        ...
        $('#Example').on('click', '.delete-btn', function () {
            ...
            if (confirm('Are you sure you want to delete this record?')) {
                $.ajax({
                    url: '<?=base_url('dashboard/admin/delete_career')?>',
                    type: 'POST',
                    data: { id: id },
                    success: function (response) {
                        const res = JSON.parse(response);
